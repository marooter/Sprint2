...
class  Payment2 extends React.Component {
  ...
  handClick(){
    ...
    idBill = this.props.state.idBill
    bank = this.props.state.selectedBanks
    date = this.props.state.date
    time = this.props.state.time

    client({method: 'GET', path: '/name/'+name+'/idBill/'+idBill+'/bank/'+bank+'/date/'+date+'/time/'+time}).done(
...
export default class Payment extends React.Component {
 ...
 renderToolbar() {
  ...
  Payment2() {
    ...
    if (this.state.idBill === '' ) {
      ons.notification.alert('กรุณากรอกข้อมูลให้ครบ');
    }else {
        client({method: 'GET', path: `/billid/${this.state.idBill.trim()}`}).done(response => {
